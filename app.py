...
# ─────────────────────────────────────────
# 主題清單永久儲存（存成 JSON 檔案，重啟不會遺失）
# ─────────────────────────────────────────
TOPICS_FILE = "custom_topics.json"

def load_topics() -> list:
    """優先從 JSON 檔案讀取，檔案不存在才用預設清單"""
    if os.path.exists(TOPICS_FILE):
        try:
            with open(TOPICS_FILE, "r", encoding="utf-8") as f:
                data = json.load(f)
            # 格式轉換：JSON 存的是 list of list，轉回 list of tuple
...
with st.sidebar:
    ...
    if "topics" not in st.session_state:
        st.session_state.topics = load_topics()  # 從 JSON 檔案讀取，重啟不會遺失
    ...
    if st.session_state.show_topic_editor:
        ...
        if c1.button("✅ 套用", use_container_width=True):
            ...
            if errors:
                ...
                save_topics(new_topics)  # 永久存檔，重啟不會遺失
                st.session_state.show_topic_editor = False
                st.success(f"✅ 已套用並永久儲存 {len(new_topics)} 個主題！")
                st.rerun()
